...
| **overhaul** | `/ina:overhaul` | 코드베이스 종합검진 + 수정 계획 |
...
> **overhaul vs think improve**: overhaul는 전체 코드베이스 자동 전수조사 + codex 병렬 검토. think improve는 사용자 인터뷰 기반 개선 방향 탐색.
...
| "전체 검토", "종합검진", "처음부터 다시", "코드 점검" | `/ina:overhaul` |
...
| think | `/ina:plan` |
| overhaul | `/ina:plan` (수정 계획을 태스크로 분해) |
| plan | `/ina:build` |
| build | `/ina:review` |
| design | `/ina:review` |
